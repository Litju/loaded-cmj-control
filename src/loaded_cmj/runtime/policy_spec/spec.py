...
The classes in this module describe what an experiment intentionally exposes. They do
not perform trusted runtime validation; the trusted runtime must independently validate
...
class PolicySpec:
    ...
    def from_dict(cls, data: Mapping[str, Any]) -> PolicySpec:
        ...
        # Public model specifications may carry provenance/reset metadata alongside the
        # participant-facing schema.  The worker only consumes the declared
        # observation/action contract; these two metadata fields are trusted
        # compatibility extensions and must not become policy-visible fields.
        allowed = {
            "spec_version",
            "protocol_version",
            "entrypoint",
