...
Policies are duck typed: policy modules do not need to inherit from a
framework base class. These protocols document the forms supported by the
runtime.
"""
...
class CallablePolicy(Protocol):
    """Callable policy used by rollout evaluators that invoke ``policy(observation)``."""
