"""Out-of-process runner for policy modules.

The rollout engine is the trusted parent. The child process and every frame it
writes are untrusted. Requests carry unpredictable IDs, responses must match
...
from loaded_cmj.runtime.policy_spec import POLICY_PROTOCOL_VERSION, PolicySpec

from loaded_cmj.runtime.errors import (
...
from loaded_cmj.runtime.observations import validate_action, validate_observation
from loaded_cmj.runtime.policy_protocol import decode_response, encode_request


class PolicyWorkerError(RuntimeError, InvalidPolicyError):
...
LOADED_CMJ_EXECUTION_SENTINEL_ENV = "LOADED_CMJ_EXECUTION_SENTINEL"


def _mark_policy_execution_started() -> None:
    sentinel = os.environ.get(LOADED_CMJ_EXECUTION_SENTINEL_ENV)
    if not sentinel:
        return
    try:
        Path(sentinel).touch()
...
_AGENT_USER = "agent"
_AGENT_USER_ENV = "LOADED_CMJ_AGENT_USER"
_AGENT_UID_ENV = "LOADED_CMJ_AGENT_UID"
_AGENT_GID_ENV = "LOADED_CMJ_AGENT_GID"
_AGENT_HOME_ENV = "LOADED_CMJ_AGENT_HOME"
...
def _isolated_python_environ() -> dict[str, str]:
    ...
    env.pop(LOADED_CMJ_EXECUTION_SENTINEL_ENV, None)
...
def _worker_environment(
    *,
    ...
    # Keep policy startup predictable in constrained runtime containers.
    ...
    for root, dirnames, filenames in os.walk(resolved.parent, followlinks=False):
        ...
        if walked > _MAX_WORKSPACE_ENTRIES:
            raise InvalidPolicyError(
                f"policy workspace has more than {_MAX_WORKSPACE_ENTRIES} "
...
_ARRAY_MARKER = "__loaded_cmj_ndarray__"
...
class PolicyWorker:
    ...
    def start(self) -> None:
        ...
        _mark_policy_execution_started()
        ...
        try:
            ...
            if isinstance(
                exc, (InvalidPolicyError, InternalEvaluationError)
            ) or not isinstance(exc, Exception):
                raise
            if _is_agent_resource_pressure(exc):
                raise PolicyWorkerError(
                    "policy worker spawn failed under configured resource limits: "
        ...
        with self._call_lock:
            ...
            try:
                ...
                if isinstance(frame, BaseException):
                    if isinstance(frame, InvalidPolicyError):
                        raise frame
                    raise PolicyProtocolError(
    ...
    def _drain_protocol(
        ...
        try:
            while True:
                ...
                # carried as policy protocol errors, not raw exceptions from
                ...
                try:
                    ...
                    return
                # Preserve pipe order. The parent decodes the first frame and
                # rejects any later frame against the next request; using
                # Queue.qsize() here races the parent consumer and can hide a
                # malformed first frame behind a later duplicate.
                responses.put(line)
        except Exception as exc:  # noqa: BLE001
            responses.put(
                PolicyProtocolError(
