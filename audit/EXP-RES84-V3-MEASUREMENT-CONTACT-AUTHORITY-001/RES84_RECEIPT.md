...
the aggregate validation report (generated count: 424 checks, 0 failed — the
...
| RES95 EM-10 comparator (RES-84A corrected) | `LEFT_FOOT_FZ < 10 N AND RIGHT_FOOT_FZ < 10 N` continuously for ≥ 0.010 s on the legal plantar per-foot wrenches (GROUND_ON_ATHLETE). ACTIVE prohibited/non-plantar floor support invalidates (INVALID) instead of masquerading as bilateral below-threshold force. Total Fz is emitted as a report field only. Diagnostic/comparability only; never defines physical takeoff, flight, the occurrence timestamp or the H2 origin |
...
| `MEASUREMENT_VALIDATION_REPORT.json` | 20 artifacts, 424 checks, 0 failed, status PASS (aggregate regenerated from the report) |
...
disagrees with the report. Current aggregate: **424 checks, 0 failed**.
